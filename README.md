...
Tinyproxy supports HTTP "Basic Authentication" via the `BasicAuth`
configuration directive.  Multiple user/password pairs can be allowed
by specifying the directive more than once:
...
When any `BasicAuth` entries are configured, the proxy requires every
client to authenticate with one of the configured credentials.  The
optional `BasicAuthRealm` directive customizes the realm string in the
